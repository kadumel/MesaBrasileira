(function () {
  ...
  function initHeroAudio() {
    const audio = hero.querySelector("[data-roda-audio-el]");
    if (!audio) return;
    ...
    async function tryPlay() {
      audio.volume = 0.42;
      audio.dataset.keepOn = "1";
      delete audio.dataset.heroPause;
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden && isAudible()) {
        audio.dataset.tabPause = "1";
        audio.pause();
      } else if (!document.hidden && audio.dataset.tabPause === "1") {
        delete audio.dataset.tabPause;
        tryPlay().catch(() => {});
      }
    });
    ...
    const unlock = () => {
      ...
      audio.muted = false;
      if (isAudible()) {
        window.setTimeout(attachScrollPause, 2500);
        return;
      }
      tryPlay()
        .then(() => {
  ...
  function attachScrollPause() {
    ...
    if (audio.dataset.scrollBound === "1") return;
    audio.dataset.scrollBound = "1";

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (audio.dataset.skipIo === "1") {
            delete audio.dataset.skipIo;
            return;
          }
          if (!entry.isIntersecting && !audio.paused) {
            audio.dataset.heroPause = "1";
            audio.pause();
          } else if (entry.isIntersecting && audio.dataset.heroPause === "1") {
            delete audio.dataset.heroPause;
            if (audio.dataset.keepOn === "1") {
              audio.muted = false;
              audio.play().catch(() => {});
            }
          }
